...
namespace Pinpaida.DataAccess
{
    public class MySqlHelper
    {
        //数据库连接字符串(web.config来配置)，可以动态更改SQLString支持多数据库.		
        public static string connectionString = ConfigurationManager.AppSettings["MySqlConnection"];
        public MySqlHelper()
        {
            //
            // TODO: 在此处添加构造函数逻辑
        ...
        public static void ExecuteSqlTran(Hashtable SQLStringList)
        {
            using (MySqlConnection conn = new MySqlConnection(connectionString))
            {
                ...
                using (MySqlTransaction trans = conn.BeginTransaction())
                {
                    ...
                    {
                        trans.Rollback();
                        throw new Exception(sqls + ",错误：" + ex.Message);
                    }
                    finally
                    {
                        cmd.Dispose();
        ...
        #region MySqlDataReader转换成DataTable
        ...
        /// <returns></returns>
        public static string LetterFilter(string txt)
        {
